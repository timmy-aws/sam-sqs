...
```bash
sqs-test$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sqs-test$ sam local start-api
sqs-test$ curl http://localhost:3000/
```
...
```bash
sqs-test$ sam logs -n HelloWorldFunction --stack-name sqs-test --tail
```
...
```bash
sam delete --stack-name sqs-test
```
